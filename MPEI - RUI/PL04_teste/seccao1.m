    ...
    while(isempty(option) | option < 4 | user == 0)
        ...
        elseif (isempty(user) || user< 1 || user>Nu)
            ...
            switch option
                ...
                case 2
                    getSuggestionsFromUsers(Nu, MinHashValue, user, Set, dic);
                case 3
                    getSuggestionsFromMovies(Nu, MinHashValue, user, Set, dic);  
                case 4
                    getPopularMovies(dic, MinHashSig, MinHashValue, Nu, Set);
                case 5
                    fprintf("EXIT\n");
                    break
                otherwise
...
function getSuggestionsFromUsers(Nu, MinHashValue, user, Set, dic)
    ...
    fprintf("Com base do user 1: \n")

    fprintf(2, 'Press any key to continue. ');
    pause;clc;

    for i = 1:length(Set{Used_User1})
        if ~ismember(Set{Used_User1}(i), Set{user})
        fprintf('%d - %s\n', i, dic{Set{Used_User1}(i)});
        end
    end
    ...
    fprintf("Com base do user 2: \n")

    fprintf(2, 'Press any key to continue. ');
    pause;clc;  % Manter a informação disponível até ao utilizador pressionar em qualquer tecla
    
    for i = 1:length(Set{SimilarUserId})
        if ~ismember(Set{SimilarUserId}(i), Set{user})
        fprintf('%d - %s\n', i, dic{Set{SimilarUserId}(i)});
        end
    end
...
%%
function getSuggestionsFromMovies(Nu, MinHashValue, user, Set, dic)
  
end
%%
function getPopularMovies(dic, MinHashSig, MinHashValue, Nu, Set)
    str = lower(input('\nWrite a String in lower case: ', 's'));
